...
module.exports = function (context) {
	...
	return function (_Component) {
		...
		function SiteInfoVolumes(props) {
			...
			_this.formatDockerPath = function (filepath) {
			...
			value: function inspectContainer() {
				...
				docker().getContainer(site.container).inspect(function (err, containerInfo) {
					...
					containerInfo.Mounts.forEach(function (mount) {
						var source = 'win32' === os.platform() ? path.resolve(mount.Source.replace('/c/', '/')) : mount.Source;
							...
							docker().createContainer({
								...
								if ('clonedImage' in site) {
									if (typeof site.clonedImage === 'string' && site.clonedImage) {
										...
										clonedImages = [].concat(_toConsumableArray(site.clonedImage));
									}
								}
